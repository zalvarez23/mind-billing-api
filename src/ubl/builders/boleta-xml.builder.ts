...
import {
  ...
  InvoiceBuildItem,
  InvoiceTotals,
} from '../interfaces/boleta-build-input.interface';

const IGV_RATE = 0.18;
const BOLETA_DOC_TYPE = '03';
const DEFAULT_TIPO_OPERACION = '0101';
...
@Injectable()
export class BoletaXmlBuilder {
  build(input: BoletaBuildInput): { xml: string; totals: InvoiceTotals } {
    const tipoOperacion = input.tipoOperacion ?? DEFAULT_TIPO_OPERACION;
    const totals = this.calculateTotals(input.items);
    const invoiceId = `${input.serie}-${input.correlativo}`;
    const linesXml = input.items
      .map((item, index) => this.buildLine(item, index + 1, input.moneda))
      .join('\n');
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
  ...
  <cbc:InvoiceTypeCode listID="${tipoOperacion}" listAgencyName="PE:SUNAT" listName="Tipo de Operacion" listURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo51">${BOLETA_DOC_TYPE}</cbc:InvoiceTypeCode>
  ...
  <cac:PaymentTerms>
    ...
    <cbc:PaymentMeansID>${input.formaPago ?? 'Contado'}</cbc:PaymentMeansID>
  </cac:PaymentTerms>
  <cac:TaxTotal>
  ...
  <cac:LegalMonetaryTotal>
    <cbc:LineExtensionAmount currencyID="${input.moneda}">${totals.subtotal.toFixed(2)}</cbc:LineExtensionAmount>
    <cbc:TaxInclusiveAmount currencyID="${input.moneda}">${totals.total.toFixed(2)}</cbc:TaxInclusiveAmount>
...
${linesXml}
  ...
  getFileBaseName(ruc: string, serie: string, correlativo: number): string {
    return `${ruc}-${BOLETA_DOC_TYPE}-${serie}-${correlativo}`;
  }
